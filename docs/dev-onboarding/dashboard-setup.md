...
- **Event chrome (#146):** Create or mark a space as **Event** (`kind`). Active events show a **Featured event** card on Home. Enter shows a stable share link + QR (kiosk URL when kiosk is on). Optional `startsAt` / `endsAt` via `PATCH /api/game-state/:id/event`.
