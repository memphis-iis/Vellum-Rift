...
/** List processed meshes from the backend (newest first). */
export async function fetchModels(limit = 100): Promise<ModelMeta[]> {
  const qs = new URLSearchParams({ limit: String(limit) });
  const res = await fetch(`${API_BASE_URL}/api/models?${qs}`, {
    headers: authHeaders(true),
  });
  const data = (await res.json().catch(() => null)) as ModelMeta[] | { error?: string } | null;
  if (!res.ok) {
